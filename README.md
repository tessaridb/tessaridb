...
[![conformance](https://img.shields.io/badge/conformance-1307%20cases-6B5FD1?style=flat-square)](crates/tessari-conformance/tests/corpus)
...
| **Queues** | `DEFINE QUEUE` — work handed out one holder at a time under a hold that lapses, first-in-first-out by identity or on the record you name, an attempt ceiling whose dead letter is a predicate rather than a second table, and a claim that is an ordinary write so it replicates, recovers and needs no lease manager | 32 | ✅ runs |
