...
use bgv_db_storage::{Transaction, Verb};
...
impl Session<'_> {
    ...
    pub(crate) fn visible_in(
        ...
        let grants = bgv_db_storage::Catalog::new(transaction).grants_for(user.id)?;
    ...
    pub fn visible(&self, store: &bgv_db_storage::Store, table: TableId) -> Result<Visible> {
