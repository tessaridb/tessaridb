...
//!
//! Every operator here answers with one, so a condition can only be
//! non-boolean when the author wrote a bare path or literal in that position.
//! `WHERE tags` is not a question with a false answer; it is a question that was
//! not finished, and an error naming what was found says so where an empty
//! result would hide it.

use bgv_db_ql::{BinaryOp, Span};
...
pub(crate) fn apply(op: BinaryOp, left: &Value, right: &Value) -> bool {
    match op {
...
mod tests {
    ...
    use bgv_db_ql::BinaryOp;
    use bgv_db_types::{Number, Value};

    use super::{apply, matches_pattern};
