...
//! whole table.
//!
//! Every fold this store has — `count`, `sum`, `mean`, `min`, `max` — can be
//! computed one value at a time. So that case wants an accumulator, not a spill:
//! a group now holds **one of these per fold occurrence**, whatever the group's
//! size, and there is nowhere left for a per-record collection to live.
//!
...
use bgv_db_ql::{Aggregate, ExprKind, Span};
...
impl Accumulator {
    ...
    ///
    /// The counter behind the claim: an accumulator holds at most one value
    /// however many it is offered, so what a group costs is set by its folds and
    /// not by its records.
    #[cfg(test)]
...
mod tests {
    use super::{Accumulator, Aggregate, Decimal, Number, Span, Value};
    ...
    #[test]
    fn a_fold_holds_at_most_one_value_however_many_it_is_offered() {
