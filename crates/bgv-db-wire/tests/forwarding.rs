...
fn replicate(leader: &Db, follower: &Db) -> u64 {
    ...
    let sent = bgv_db_backup::write_from(leader.store(), &mut carried, from).unwrap();
    let applied = bgv_db_backup::bootstrap(follower.store(), &mut carried.as_slice()).unwrap();
    assert_eq!(
