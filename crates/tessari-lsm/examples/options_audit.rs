...
//!
//! ```text
//! cargo run -p tessari-lsm --example options_audit -- <store-path>
//! ```
//!
...
use tessari_lsm::{LsmBackend, StoreConfig, effective_options_files};
...
fn main() -> ExitCode {
    ...
    let (path, temporary) = match argument {
        ...
        None => (
            std::env::temp_dir().join(format!("tessari-options-audit-{}", std::process::id())),
            true,
        ),
    };
