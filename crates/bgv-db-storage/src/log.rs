...
use bgv_db_encoding::{AppliedPositionKey, LogKey, LogRecord, RecordKey, StoreKey, StoreValue};
...
mod tests {
    ...
    use bgv_db_encoding::{Mutation, RecordValue};
    use bgv_db_kv::{Keyspace, Precondition};
    ...
    fn one_batch_carries_the_log_record_the_versions_and_the_position() {
        ...
        let keyspaces: Vec<Keyspace> = batch
            .ops()
            .iter()
            .map(bgv_db_kv::WriteOp::keyspace)
            .collect();
