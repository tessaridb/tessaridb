...
//!
//! Being explicit about this is the point, because both gaps are easy to read
//! past and neither is closed by the test passing.
//!
...
//!
//! **It does not exercise a divergent flush.** At this record count nothing has
//! been flushed out of memory yet, so recovery replays the log — where a batch
//! is one record and cross-region atomicity is trivially preserved. Atomic flush
//! matters in the case this test never reaches: one region flushed, another not,
//! and the log advanced past the second. That the setting is *in effect* is
//! asserted against the option set the engine itself wrote; that it *holds under
//! a divergent flush* needs a crash placed inside the flush, and is an open item
