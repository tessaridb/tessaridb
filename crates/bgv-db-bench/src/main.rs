...
//!
//! ```text
//! cargo run -p bgv-db-bench --release                     every workload, in memory
//! cargo run -p bgv-db-bench --release -- --list           what there is to run
//! cargo run -p bgv-db-bench --release -- --workload write one of them
//! cargo run -p bgv-db-bench --release -- --backend disk   against the on-disk engine
//! cargo run -p bgv-db-bench --release -- --baseline benchmarks/today.md
//! ```
//!
...
const USAGE: &str = "\
usage: bgv-db-bench [--backend memory|disk] [--workload <name>] [--baseline <path>] [--list]";
...
impl Scratch {
    fn new(label: &str) -> io::Result<Self> {
        ...
        path.push(format!("bgv-db-bench-{label}"));
        if path.exists() {
