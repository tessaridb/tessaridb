...
pub(crate) struct Envelope {
    /// The script to run.
    pub script: String,
    /// Each parameter's value, written in TessariQL.
...
impl Reader {
    ...
    /// assembled: a parameter's value is TessariQL text, and a caller writing an
    ...
    fn strings(&mut self) -> Result<BTreeMap<String, String>, String> {
        ...
        loop {
            ...
            let written = self.string().map_err(|reason| {
                format!("a parameter's value is written in TessariQL, as a string ({reason})")
            })?;
...
mod tests {
    ...
    fn what_is_wrong_is_said_rather_than_guessed() {
        for (body, expected) in [
            ...
            (
                r#"{"script":"a","parameters":{"n":3}}"#,
                "written in TessariQL",
            ),
