...
fn taken() -> Vec<u8> {
    ...
    let written = tessari_backup::write(&store, &mut bytes).unwrap();
...
fn a_backup_names_the_build_that_wrote_it() {
    let bytes = taken();
    let checked = tessari_backup::verify(&mut bytes.as_slice()).unwrap();
...
fn a_restore_reports_the_build_the_file_came_from() {
    let bytes = taken();
    let (_, into) = store();
    let restored = tessari_backup::read(&into, &mut bytes.as_slice()).unwrap();
...
fn a_backup_from_a_newer_build_is_refused_before_anything_is_applied() {
    ...
    let (_, into) = store();
    let refused = tessari_backup::read(&into, &mut bytes.as_slice()).unwrap_err();
