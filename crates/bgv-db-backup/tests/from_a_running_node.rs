...
fn a_backup_taken_through_the_language_verifies_and_restores() {
    ...
    bgv_db_backup::read(&restored_store, &mut held.as_slice()).unwrap();
...
fn an_increment_carries_what_happened_after_it_and_the_two_together_are_the_whole() {
    ...
    let at = bgv_db_backup::verify(&mut base.as_slice()).unwrap().tail;
    ...
    let restored_store = store();
    bgv_db_backup::read(&restored_store, &mut base.as_slice()).unwrap();
    bgv_db_backup::read(&restored_store, &mut increment.as_slice()).unwrap();
    let mut restored = Session::new(&restored_store);
    restored
        .run("USE NAMESPACE prod; USE DATABASE shop;")
        .unwrap();
