...
//!
//! The nearest existing answer is `bgv-db-lsm`'s
...
/// already owed elsewhere (wave 56, `bgv-db-http`) and belongs in that wave.
const LEADER: &str = "\
...
fn a_follower_that_was_down_catches_up_on_what_it_missed_without_replaying() {
    ...
    {
        let follower = durable(&path);
        bgv_db_backup::bootstrap(&follower, &mut prefix.as_slice()).unwrap();
    }
    ...
    let sent = bgv_db_backup::write_from(&held, &mut missed, resume_from(&reopened)).unwrap();
    assert!(
        sent.records > 0,
        "the leader had nothing to send, so this test proves nothing"
    );

    let caught_up = bgv_db_backup::bootstrap(&reopened, &mut missed.as_slice()).unwrap();
