...
| an order served **under a `WHERE`** | the condition and the order have to be served by one index. The gathering-by-index restructuring this row used to wait on is **built** — a condition is now offered to every index whose leading field it names, and each is asked what the whole condition fixes for it — so what is left here is the order itself, which inherits every boundary rule a bounded index-served order already has: absences sort somewhere and the index does not hold them, ties break by identity while the key is `value ++ identity`, and an order taken from an index must not be served over a field the caller's grant excludes. Today a filtered read narrows by the condition and sorts what it found, which is correct and costs a sort. §5 |
